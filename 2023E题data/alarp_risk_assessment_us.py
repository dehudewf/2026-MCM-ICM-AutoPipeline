...
class ALARPRiskAssessmentUS:
    """
    ALARP（As Low As Reasonably Practicable）风险评估模型
    用于评估美国境内四个地点的光污染影响风险
    """
    ...
    def load_wsm_scores(self) -> pd.DataFrame:
        ...
        加载WSM得分数据
        """
        filepath = os.path.join(self.data_dir, 'wsm_results_us', 'wsm_scores_us.csv')
    ...
    def classify_risk_level(self, risk_value: float) -> Tuple[int, str, str]:
        """
        根据风险值划分风险等级
    ...
    def print_alarp_thresholds(self):
        ...
        print("-" * 100)
        print("ALARP是风险管理中常用的评估框架，将风险分为三个区域：")
        print()
    ...
    def print_risk_assessment_results(self, risk_data: pd.DataFrame):
        ...
        """
        print("\n" + "=" * 100)
        print("美国境内四个地点风险评估结果")
        print("=" * 100)
        
        print(f"\n{'地区':<25} {'州':<10} {'WSM得分':<12} {'风险值':<12} {'风险百分比':<12} {'风险等级':<10} {'风险类别':<15}")
        print("-" * 100)
        
        for idx in range(len(risk_data)):
            region_name = risk_data.loc[idx, 'Region_Name']
            state = risk_data.loc[idx, 'State']
            ...
            print(f"{region_name:<25} {state:<10} {wsm_score:<12.4f} {risk_value:<12.4f} {risk_percentage:<12.2f}% {risk_level:<10} {risk_category:<15}")
    ...
    def save_results(self, risk_data: pd.DataFrame):
        ...
        保存结果
        """
        output_dir = os.path.join(self.data_dir, 'alarp_results_us')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        risk_file = os.path.join(output_dir, 'alarp_risk_assessment_us.csv')
...
def main():
    ...
    主函数
    """
    assessor = ALARPRiskAssessmentUS()
