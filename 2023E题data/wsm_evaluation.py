...
class WSMEvaluatorator:
    ...
    def load_real_locations_data(self) -> pd.DataFrame:
        ...
        加载真实地点数据
        """
        filepath = os.path.join(self.data_dir, 'real_locations_data.csv')
    ...
    def min_max_normalize(self, data: pd.DataFrame,
        ...
        if exclude_cols is None:
            exclude_cols = ['Region', 'Region_Name', 'Region_Type']
    ...
    def print_comparison(self, original_data: pd.DataFrame,
        ...
        indicators = [col for col in original_data.columns 
                    if col not in ['Region', 'Region_Name', 'Region_Type']]
    ...
    def save_results(self, normalized_data: pd.DataFrame,
        ...
        保存结果
        """
        output_dir = os.path.join(self.data_dir, 'wsm_results')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        normalized_file = os.path.join(output_dir, 'normalized_real_locations.csv')
        normalized_data.to_csv(normalized_file, index=False, encoding='utf-8-sig')
        print(f"\n归一化数据已保存: {normalized_file}")

        scored_file = os.path.join(output_dir, 'wsm_scores.csv')
        ...
        category_file = os.path.join(output_dir, 'category_scores.csv')
    ...
    def run_full_evaluation(self):
        ...
        print("归一化完成！")
        print("\n归一化数据预览:")
        print(normalized_data[['Region', 'Region_Name', 
        ...
        print(ranked[['Region_Name', 'WSM_Score', 'WSM_Rank']])
